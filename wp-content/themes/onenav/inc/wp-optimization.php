...
if( io_get_option('disable_feed') ) {
    function io_disable_feed() {
        wp_die('<h1>'.sprintf(__('Feed已经关闭, 请访问网站<a href="%s">首页' , 'i_theme'), esc_url(home_url())).'</a>!</h1>');
    }
...
add_filter('pre_get_avatar_data', function($args, $id_or_email){
    ...
    switch (io_get_option('gravatar')){
        ...
        case "garav":
            ...
            break;
        case "chinayes":
            $url    = 'http://gravatar.wp-china-yes.net/avatar/'.$email_hash;
            break;
        case "geekzu":
            $url    = 'http://sdn.geekzu.org/avatar/'.$email_hash;
            break;
        default:
            $url    = 'http://sdn.geekzu.org/avatar/'.$email_hash;
    }
...
if(io_get_option('remove_dns_prefetch'))
