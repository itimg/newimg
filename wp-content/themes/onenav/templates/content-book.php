            ...
            <div class="row site-content py-4 py-md-5 mb-xl-5 mb-0 mx-xxl-n5">
                ...
                <div class="col mt-4 mt-sm-0">
                    <div class="site-body text-sm">
                        ...
                        ?>
                        <h1 class="site-name h3 my-3"><?php echo $booktitle.$journal ?></h1>
                        <div class="mt-n2"> 
                            ...
                            <div class="site-go mt-3">
                                <?php if($buy_list = get_post_meta(get_the_ID(), '_buy_list', true)) foreach ($buy_list as  $value) {
                                    echo '
                                    <a target="_blank" href="'.home_url().'/go/?url='.base64_encode($value['url']).'" class="btn"  title=""><span>'.$value['term'].'(￥'.($value['price']?:'0').')</span></a>';
                                }
                                if( $down_list ) { ?>
                                <a href="javascript:" class="btn btn-arrow qr-img"  title="" data-id="0" data-toggle="modal" data-target="#book-down-modal"><span><?php echo __('下载资源','i_theme') ?><i class="iconfont icon-down"></i></span></a>
                                <?php } ?>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="down_btn_list my-4">
                                ...
                                <?php  
                                    for($i=0;$i<count($down_list);$i++){
                                        ...
                                        echo '<div class="col-4 col-md-3 text-right"><a class="btn btn-danger custom_btn-d py-0 px-1 mx-auto down_count text-sm" href="'.home_url().'/go/?url='.base64_encode($down_list[$i]['url']).'" target="_blank" data-id="'. get_the_ID() .'" data-action="down_count" data-clipboard-text="'.($down_list[$i]['tqm']?:'').'" data-mmid="down-mm-'.$i.'">'.$down_list[$i]['name'].'</a></div>';
