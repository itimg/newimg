...
if ( ! defined( 'ABSPATH' ) ) { exit; }  ?>
    		...
    		<div class="card-post list-item">
                ...
                <div class="list-content">
                    <div class="list-body">
                        <a href="<?php the_permalink(); ?>" <?php echo new_window() ?> class="list-title text-md overflowClip_2">
                        <?php show_sticky_tag( is_sticky() ) . show_new_tag(get_the_time('Y-m-d H:i:s')) ?><?php the_title(); ?>
                        </a>
                    </div>
                    <div class="list-footer">
                        <div class="d-flex flex-fill align-items-center">
                            <a href="<?php echo get_author_posts_url( get_the_author_meta( 'ID' ) ) ?>" class="flex-avatar mr-1" <?php echo new_window() ?>>
                            <?php echo get_avatar( get_the_author_meta('email'), '20' ); ?>               
                            </a>
                            <a href="<?php echo get_author_posts_url( get_the_author_meta( 'ID' ) ) ?>" class="d-none d-md-inline-block text-xs" <?php echo new_window() ?>><?php echo get_the_author() ?></a>
